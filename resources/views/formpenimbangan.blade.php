            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="col-md-10 col-lg-12">
                        <form action="{{ route('form.add') }}" id="form-balita" method="POST">
                            @csrf
                            <div class="form-group">
                                <input type="hidden" name="id_dusun" class="form-control" placeholder="Masukkan tinggi badan..." value="{{ $idDusun }}">
                            </div>
                            <div class="form-group">
                                <label>Nama Balita :</label>
                                <div class="input-group mb-3">
                                    <select name="id_balita" id="id_balita" class="form-control" style="-webkit-appearance: none;
                                    -moz-appearance: none;
                                    appearance: none;" disabled>
                                    </select>
                                    <div class="input-group-append">
                                        <button type="button" class="input-group-text btn btn-primary text-white" data-toggle="modal" data-target="#pilihBalita">Pilih Balita</button>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" placeholder="Masukkan tinggi badan..." name="berat_badan">
                            </div>
                            <div class="form-group">
                                <label>Tinggi Badan</label>
                                <input type="number" class="form-control" placeholder="Masukkan berat badan..." name="tinggi_badan">
                            </div>
                            ...
                            <div class="modal-body">
                                <div class="table-responsive">
                                    <table id="add-row" class="display table table-striped table-hover" >
                                        ...
                                        <tbody>
                                            @foreach ($newData as $index => $db)
                                                <tr>
                                                    <td class="text-center">{{ $index + 1 }}</td>
                                                    <td class="text-center">{{ $db->nama_balita }}</td>
                                                    <td class="text-center">
                                                        <button type="button" data-dismiss="modal" class="btn btn-info btn-sm pilih-balita" data-id="{{ $db->id }}" data-nama="{{ $db->nama_balita }}" >Pilih</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
